...
impl Connector {
    ...
    pub(crate) fn subscribe<T>(&self, subscriber: Sender<T>) -> Result<usize, ConnectionManagerError>
    where
        T: From<ConnectionManagerNotification> + Send + 'static
    {
        ...
        self.sender
            .send(CmMessage::Request(CmRequest::Subscribe {
                sender,
                callback: Box::new(move |notification| subscriber.send(T::from(notification)).map_err(Box::from))
            }))
            .map_err(|_| {
                ConnectionManagerError::SendMessageError(
                    "The connection manager is no longer running".into(),
