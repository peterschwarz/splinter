...
/// Constructs a new WsAdminServiceEventClient.
#[derive(Default)]
pub struct WsAdminServiceEventClientBuilder {
...
impl WsAdminServiceEventClientBuilder {
    ...
    ///
    /// This field is required by the final WsAdminServiceEventClient.
    ...
    /// Build the runnable (but not started) WsAdminServiceEventClient.
    ///
    /// # Errors
    ///
    /// Returns an InvalidStateError if any required fields are missing.
    pub fn build(self) -> Result<RunnableWsAdminServiceEventClient, InvalidStateError> {
        let root_url = self
            .root_url
        ...
        Ok(RunnableWsAdminServiceEventClient {
            ws_runtime,
            root_url,
            event_type,
            authorization,
...
/// A configured, but not yet started WsAdminServiceEventClient.
pub struct RunnableWsAdminServiceEventClient {
...
impl RunnableWsAdminServiceEventClient {
    /// Starts the WsAdminServiceEventClient.
    ///
    /// # Errors
    ///
    /// Returns an InternalError if the client is unable to start.
    pub fn run(self) -> Result<WsAdminServiceEventClient, InternalError> {
        let Self {
            ws_runtime,
            root_url,
            event_type,
...
pub struct WsAdminServiceEventClient {
    ws_runtime: WsRuntime,
    event_receiver: Receiver<Result<AdminServiceEvent, WebSocketError>>,
}

impl ShutdownHandle for WsAdminServiceEventClient {
...
impl Drop for WsAdminServiceEventClient {
    fn drop(&mut self) {
        self.signal_shutdown();
    }
}

impl AdminServiceEventClient for WsAdminServiceEventClient {
    /// Non-blocking
    fn try_next_event(&self) -> Result<AdminServiceEvent, TryNextEventError> {
        let evt_result = self.event_receiver.try_recv().map_err(|e| match e {
            TryRecvError::Empty => TryNextEventError::Empty,
            TryRecvError::Disconnected => TryNextEventError::Disconnected,
        })?;

        evt_result
            .map_err(|e| TryNextEventError::InternalError(InternalError::from_source(Box::new(e))))
    }
...
impl ParseBytes<AdminServiceEvent> for AdminServiceEvent {
    fn from_bytes(bytes: &[u8]) -> Result<AdminServiceEvent, ParseError> {
        ...
        let (proposal, event_type) = match json_event.admin_event {
            ...
            CircuitDisbanded(proposal) => (proposal, EventType::CircuitReady),
        };
